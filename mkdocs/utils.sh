...
function setupPython () {
    ...
    installPath="${rootPath}/bin/${platform}/${pythonName}"
     ...
     if command -v $pythonCmd &> /dev/null; then
        ...
        if [ "$platform" == "macos" ] || [ "$platform" == "macos-arm" ]; then
            ...
            if [ "$platform" == "macos-arm" ]; then
                ...
                # Download $assetStorageUrl $downloadPath "python3.7.12-osx64.tar.gz" "${platform}/${pythonDir}" "Downloading Python interpreter..."
                # cp -R "${downloadPath}/${platform}/${pythonDir}" "${rootPath}/bin/${platform}"
    ...
    if [ -d  "${installPath}/venv" ]; then
        ...
        if [ "$platform" == "macos" ] || [ "$platform" == "macos-arm" ]; then
            ...
            if [ "${verbosity}" == "quiet" ]; then
                ...
                sudo apt install python${pythonVersion}-venv >/dev/null 2>/dev/null &
                spin $!
                writeLine "Done" $color_success
            else
...
function installPythonPackages () {
    ...
    virtualEnv="${rootPath}/bin/${platform}/${pythonName}/venv"
